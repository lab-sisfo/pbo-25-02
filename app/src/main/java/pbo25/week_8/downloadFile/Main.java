    ...
    public static void main(String[] args) {
        ...
        uiExecutor.submit(() -> {
            ...
            while (!allDone) {
                try {
                    ...
                    long elapsedTime = (System.currentTimeMillis() - uiStartTime) / 1000;
        ...
        for (int i = 1; i <= totalFiles; i++) {
            ...
            downloadExecutor.submit(() -> {
                try {
                    // Simulasikan proses download dengan jeda waktu acak (1-3 detik)
                    Random random = new Random();
                    int duration = random.nextInt(3) + 1; // Random 1-3 detik
                    Thread.sleep(duration * 1000);

                    // Catat hasil download
        ...
        try {
            downloadExecutor.awaitTermination(Long.MAX_VALUE, TimeUnit.NANOSECONDS); //long Max_value : tdk ada batas waktu, program menunggu selama yang diperlukan
        } catch (InterruptedException e) {                                            //nanoseconds: waktu max value diinterpretasi dalam satuan nanodetik
            Thread.currentThread().interrupt();
        } // Interupsi thread: satu thread dapat memberi sinyal kepada thread lain bahwa ia harus berhenti melakukan pekerjaannya atau menunggu.

        // TODO (8)
        // Ketika semua proses download sudah selesai:
        allDone = true; // Tandai bahwa semua download sudah selesai
